    ...
    public Set<Interpreter> findAvailable(LocalDateTime start, LocalDateTime end) throws SQLException {
        String query = String.format(
                "SELECT * FROM " + TABLE + " WHERE " + FIELD_ID + " <> ALL (" +
                        ...
                        " WHERE (tm." + DAOPunctualTimeSlot.FIELD_START_TIME + " <= ? AND tm." + DAOPunctualTimeSlot.FIELD_END_TIME + " >= ?  AND m." + DAOMission.FIELD_STATE + " = ANY (?, ?))" +
        ...
        try {
            ...
            statement.setInt(4, MissionState.REGULAR.getValue());
